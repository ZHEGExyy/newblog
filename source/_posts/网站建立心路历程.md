...
网站配置 favicon  Site  page
博客插入 图片 视频 音频 链接emoji
...
cards主题有自带的几个`page`相关设置，例如: 标签`tags` 分类`categories` 友链`links`
cards主题官方教程：<https://theme-cards.ichr.me/pages/>
若是需要自定义的增加`page`，可以参考下方教程，但页面只能与文章页相同排布
...
```
（3）
`hexo s` 预览可以看到首页界面多出了一个链接按钮，点击可到达文章页面
------------------------------------------
...
**注意** ：**sm.ms**网站上传后有专门的markdown链接可以复制
